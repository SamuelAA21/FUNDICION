...
class CtrlHorno
{
    ...
    {
        $this->hornoDao = new HornoDAO();
        $this->combustibleDao = new CombustibleDAO();
    }

    public function read()
    {
        $combustiblesRows = $this->combustibleDao->getAll();
        $combustibles = [];

        foreach ($combustiblesRows as $row) {
            $combustibles[] = [
                'id' => (int)($row['com_id'] ?? 0),
    ...
    public function data()
    {
        ...
        $rows = $this->hornoDao->getAll();
        $array = ['data' => []];

        foreach ($rows as $row) {
            $hor_id = (int)($row['hor_id'] ?? 0);
    ...
    public function one()
    {
        ...
        echo json_encode($this->hornoDao->getById($hor_id) ?? []);
    }
